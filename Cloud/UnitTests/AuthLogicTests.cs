...
public class AuthLogicTests
{
    ...
    // {
    //     // Arrange
    //     var user = new User { Email = "hans@prins.dk", Password = "123" };
    //     var userLoginDto = new UserLoginDto { User = user };
    //     var mockAsyncCursor = new Mock<IAsyncCursor<User>>();
    //     mockAsyncCursor.Setup(_ => _.Current).Returns(new List<User> { user });
    //     mockAsyncCursor
    ...
    //     Assert.AreEqual("User logged in successfully.", result.Message);
